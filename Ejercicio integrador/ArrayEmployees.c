...
int addEmployee(eEmployee *emp, int len,eSector *auxSector, int lenSector)//, int id, char nombre[],char apellido[],float salario,int sector)

{
    ...
    if(index!=-1)
    {
        ...
        getString(emp[index].nombre,"\nIngrese nombre: \n","\nERROR\n",1,51);
        getString(emp[index].apellido,"\nIngrese apellido : \n","\nERROR\n",1,51);
        getFloat(&emp[index].salario,"\nIngrese salario : \n","\nERROR\n",1,200000);
        system("cls");
...
void printEmployee(eEmployee emp,eSector sec[], int lenSec, int formato)
{
    ...
    for(i=0;i<lenSec;i++)
    {
        ...
        if(formato==0)
        {
            printf("\n%10d %10s %10s %10.2f %15s\n", emp.id, emp.nombre, emp.apellido, emp.salario, auxSec);
